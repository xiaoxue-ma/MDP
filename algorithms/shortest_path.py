...
class AStarShortestPathAlgo():
            ...
            for n in neighbours:
                ...
                if n.visited:
                    if new_f < n.get_f():
                        n.set_g(new_g)
                        n.parent = cur_node
                        n.ori = AbsoluteOrientation.get_ori_at_dest(start_pos=(cur_node.x, cur_node.y),
                                                                    dest_pos=(n.x, n.y))
                else:  # not visited
                    n.parent = cur_node
                    n.set_g(new_g)
    ...
    def compute_g_value(self, cur_node, target_node):
        ...
        target_ori = AbsoluteOrientation.get_ori_at_dest(start_pos=(cur_node.x, cur_node.y),
                                                         dest_pos=(target_node.x, target_node.y))
    ...
    def get_neighbour_nodes(self, node):
        ...
        for pos in neighbour_rel_pos:
            ...
            if not self._map_ref.is_out_of_arena(x, y) and self._nodes[y][x] is not None:
