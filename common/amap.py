    ...
    def _load_map_file(self, filename):
        ...
        for i in range(len(explored)):
            ...
            if explored[i] == self.bit_unknown:
                ...
                cleared_cur_index += 1
        if (any([True if ls[y][x] is None else False
...
class MapRef(BitMapIOMixin, MapSetting, BasePublisher):
    ...
    def reset(self, x=None, y=None, default=None):
        ...
        if x and y and default:
            ...
            self._map_ref = [[self.DEFAULT_CELL_VALUE for _ in range(self.DEFAULT_MAP_SIZE_X)] for __ in
                             range(self.DEFAULT_MAP_SIZE_Y)]
        self._update_size()
        self.notify()

    def get_unknown_percentage(self):
        map_size = self.size_y * self.size_x
        num_unknown_cells = sum(
            [1 for x in range(self.size_x) for y in range(self.size_y) if self.get_cell(x, y) == self.UNKNOWN])
    ...
    def get_surrounding_pos(self, x, y):
        ...
        return [(x + i, y + j) for i in range(-1, 2) for j in range(-1, 2) if
                (i != 0 or j != 0) and not self.is_out_of_arena(x + i, y + j)]

    def get_along_wall_pos(self):
        """return a list of positions along the wall"""
        return [(x, y) for x in range(self.size_x) for y in range(self.size_y) if
                x == 0 or x == self.size_x - 1 or y == 0 or y == self.size_y - 1]
...
class MapUI(BaseObserver):
    ...
    Observer of MapRef
    """
    CELL_COLORS = {MapRef.OBSTACLE: "blue", MapRef.CLEAR: "green", MapRef.START_ZONE: "yellow",
                   MapRef.END_ZONE: "yellow", MapRef.UNKNOWN: "grey"}
    CELL_SIZE = 2
